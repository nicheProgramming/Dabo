...
def placeBet(slotsBetOn: int, player: Player):
    # Store [slot bet on, amount bet on slot] x 3
    placedBets = [[-1, -1], [-1, -1], [-1, -1]]
    index = 0
    slotsBetOn = int(slotsBetOn)
    debugBetSlots = [7, 19, 31]
    if player.creditCount < maxBet:
        if player.creditCount >= maxBet * 3:
            debugBetAmount = maxBet
        if player.creditCount / 3 >= minBet:
            debugBetAmount = int(player.creditCount / 3)
        elif player.creditCount / 2 >= minBet:
            debugBetAmount = int(player.creditCount / 2)
        else:
            debugBetAmount = player.creditCount
    else:
        debugBetAmount = minBet

    while slotsBetOn > 0 and player.creditCount > minBet:
            ...
            if placedBets[evalIndex][0] != -1:
                if placedBets[evalIndex][0] == slot:
                    print(
                        "Slot "
                        + str(slot)
                        + " is already bet on, please choose another."
                    )
                    continue
                else:
                    evalIndex -= 1
        ...
        elif player.creditCount < minBet:
...
class Payouts:
    ...
    def calculatePayouts(self):
        ...
        for index, result in enumerate(self.results):
            ...
            if (
                result[0][0] == "swirl"
                and result[1][0] == "swirl"
                and result[2][0] == "swirl"
            ):
            ...
            # If result is three quarks
                ...
                result[0][0] == "quark"
                and result[1][0] == "quark"
                and result[2][0] == "quark"
            ):
            ...
            # If three shapes match
                ...
                # if two counts match
                    ...
                    or result[1][1] == result[2][1]
                    or result[0][1] == result[2][1]
                ):
                    # If both colors match
                    if result[1][2] == result[2][2]:
                        # This is a 3x shape 2x count 2x color three of a kind with 150% payout
                        print("Three of a Kind!")
                        _3shape2count2color3OAK += 1
                        payouts[index] = self.bets[index][1] * 1.5
                    else:
            ...
            # If two shapes match
                ...
                or result[1][0] == result[2][0]
                or result[0][0] == result[2][0]
            ):
                # if three counts match
                if result[0][1] == result[1][1] and result[1][1] == result[2][1]:
                    # if both colors match
                ...
                # if two counts match
                    ...
                    if result[1][2] == result[2][2]:
                        ...
                        _2shape2count2color2OAK += 1
                        print("Two of a Kind!")
                        payouts[index] = self.bets[index][1] * 0.2
                    else:
                        # This is a 2x shape 2x count two of a kind with 15% payout
                        print("Two of a Kind!")
                        _2shape2count2OAK += 1
                        payouts[index] = self.bets[index][1] * 0.15
                # if both colors match
                elif result[1][2] == result[2][2]:
                ...
                # if two shapes are quarks
                    ...
                    (result[0][0] == "quark" and result[1][0] == "quark")
                    or (result[1][0] == "quark" and result[2][0] == "quark")
                    or (result[0][0] == "quark" and result[2][0] == "quark")
                ):
                ...
                # if two shapes are ds9
                    ...
                    (result[0][0] == "ds9" and result[1][0] == "ds9")
                    or (result[1][0] == "ds9" and result[2][0] == "ds9")
                    or (result[0][0] == "ds9" and result[2][0] == "ds9")
                ):
                    ...
                    print("Two of a Kind!")
                    _2shape2OAK += 1
                    payouts[index] = self.bets[index][1] * 0.1
            # if two count match
            elif (
                result[0][1] == result[1][1]
                or result[1][1] == result[2][1]
